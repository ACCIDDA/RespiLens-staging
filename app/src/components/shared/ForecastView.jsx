...
/**
 * Shared forecast visualization component used by COVID19View, RSVView, and potentially others
 * Handles forecast data visualization with Plotly charts, including:
 * - Ground truth data display
 * - Model predictions with confidence intervals
 * - Interactive zooming and range selection
...
const ForecastView = ({
  ...
  // Refs to hold latest values for Plotly button callbacks
  ...
  /**
   * Calculate appropriate y-axis range based on visible data in x-range
   */
  const calculateYRange = useCallback((data, xRange) => {
    if (!data || !xRange || !Array.isArray(data) || data.length === 0 || !selectedTarget) {
      return null;
    }
  ...
  /**
   * Process forecast data into Plotly traces
   */
  const projectionsData = useMemo(() => {
    ...
    if (!groundTruthValues) {
      logger.warn(`Ground truth data not found for target: ${selectedTarget}`);
      return [];
    }

    // Ground truth trace (observed data)
    ...
    // Model forecast traces with confidence intervals
    const modelTraces = selectedModels.flatMap(model =>
      selectedDates.flatMap((date, dateIndex) => {
        ...
        const sortedPredictions = Object.values(forecast.predictions || {})
        ...
        sortedPredictions.forEach((pred) => {
          ...
          if (val_50 !== null && val_025 !== null && val_975 !== null && val_25 !== null && val_75 !== null) {
            ...
            logger.warn(`Missing quantiles for model ${model}, date ${date}, target ${selectedTarget}, prediction date ${pred.date}`);
          }
        });
  ...
  /**
   * Determine which models have data for the current selection
   */
  ...
  // Reset x-axis range when target changes
  useEffect(() => {
    setXAxisRange(null);
  }, [selectedTarget]);
  ...
  /**
   * Handle user interactions with the plot (zooming, range changes)
   */
  ...
  /**
   * Plotly config with custom reset button
   */
