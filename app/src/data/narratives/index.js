...
const extractMetadata = (content, id) => {
  ...
  if (parts.length >= 3) {
    ...
    return {
      ...
      description:
        metadata.abstract || metadata.description || "No description available",
...
export const searchNarratives = (
  ...
  if (filterTag) {
    filtered = filtered.filter((narrative) =>
      narrative.tags.includes(filterTag),
    );
  }
