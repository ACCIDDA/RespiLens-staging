"""
Class to process Hubverse data (pd.DataFrame) into RespiLens projections-style JSON output.
"""
...
class HubverseDataProcessor:
    def __init__(
            ...
            hub: str = Literal['rsv', 'covid19', 'flusight']):
        """
        Initialize a HubverseDataProcessor object.

        Args:
            data: Hubverse forecast/model data
            locations_data: Location metadata for a hub
            target_data: Target/ground truth data for a hub
            hub: Which hub the data comes from (rsv, covid19, or flusight)
        """
        self.output_dict = {}
        self.df_data = data
        self.locations_data = locations_data
        self.target_data = target_data
        if hub not in ['rsv', 'covid19', 'flusight']:
            raise ValueError(f"Hub must be one of 'rsv', 'covid19', 'flusight'. Received: {hub}")
        self.hub = hub 

        # Group Hubverse data by loc
        logger.info(f"Building individual {self.hub} JSON files...")
        locations_gbo = self.df_data.groupby('location')
        for loc in list(locations_gbo.groups.keys()): # loc is FIPS code
            location_abbreviation = get_location_info(
                location_data=self.locations_data,
                location=loc,
                value_needed='abbreviation'
            )
            file_name = f"{location_abbreviation}_{self.hub}.json"
    ...
    def _build_ground_truth_key(self, df: pd.DataFrame) -> dict:
        ...
        # PATCH, if flu data, rename 'target_end_date' as 'date'
        if self.hub == 'flusight':
            filtered_target_data.rename(columns={'target_end_date': 'date'}, inplace=True)
        ...
        # Select only the most recently updated record for each week
        truth = filtered_target_data.sort_values('as_of').drop_duplicates(subset=['date'], keep='last')

        # Filter for the relevant season (can change)
        truth = truth[truth['date'] >= pd.Timestamp('2023-10-01')]
        if truth.empty:
            raise ValueError("No dates for relevant season (later than 2023-10-01) were found in input target data.")

        # Pivot data to align all targets against a common date index 
        # Automatically fill missing observations with NaN, then replace for JSON compatabilitly 
        pivot_truth = truth.pivot(index='date', columns='target', values='observation')
        pivot_truth = pivot_truth.replace({np.nan: None})

        # Sort by date index to ensure chronological order
        pivot_truth.sort_index(inplace=True)

        # Build the final ground_truth dictionary from the pivoted data
        ground_truth = {
            "dates": pivot_truth.index.strftime('%Y-%m-%d').tolist()
        }

        # Add each target's observation list to the dictionary
        for target_column in pivot_truth.columns:
            ground_truth[target_column] = pivot_truth[target_column].tolist()
