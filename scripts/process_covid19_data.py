...
class COVID19Preprocessor:
    """Preprocessor for CDC COVID-19 Forecast Hub data."""
    def __init__(self, base_path: str, output_path: str, demo_mode: bool = False):
        ...
        self.demo_models = ['CovidHub-ensemble', 'Google_SAI-Ensemble']
        self.all_models = set()

        # Define paths
        self.model_output_path = self.base_path / "model-output"
        self.target_data_path = self.base_path / "target-data/covid-hospital-admissions.csv"
        ...
        # Cache file listings
        self.model_files = {
            model_dir.name: list(model_dir.glob("*.csv"))
    ...
    def _validate_paths(self):
        """Ensure that all required paths exist."""
        required_paths = {
    ...
    def load_ground_truth(self) -> Dict:
        """Load and process ground truth data for COVID-19 hospitalizations."""
        if self.ground_truth is None:
            logger.info("Loading ground truth data...")
            
            # Load the locations data to get population counts
            locations_df = self.load_locations()
            population_map = locations_df.set_index('location')['population']

            # Filter relevant dates and sort
            ...
            for location in df['location'].unique():
                ...
                population = population_map.get(location)
                if population and population > 0:
                    # Rate per 100,00 people
                    loc_data['weekly_rate'] = (loc_data['value'] / population) * 100000
                else:
                    # If no population data, fill rates with None
                    loc_data['weekly_rate'] = None

                # Add rates to the output dictionary 
    ...
    def read_model_outputs(self) -> Dict:
        ...
        def process_file(file_info):
            ...
            try:
                # Read file (suffix irrelelvent; all are .csv)
    ...
    def create_visualization_payloads(self):
        ...
        # Create covid19 subdirectory
        payload_path = self.output_path / "covid19"
        ...
        for _, location_info in tqdm(locations.iterrows(), desc="Creating location payloads"):
            ...
            with open(payload_path / f"{location_abbrev}_covid19.json", 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description='Process COVID-19 forecast data for visualization')
    parser.add_argument('--hub-path', type=str, default='./covid19-forecast-hub', 
                        help='Path to covid19-forecast-hub repository')
    parser.add_argument('--output-path', type=str, default='./processed_data', 
                        help='Path for output files')
    parser.add_argument('--demo', action='store_true', 
                        help='Run in demo mode with only CovidHub-ensemble and Google_SAI-Ensemble')
    ...
    try:
        ...
        preprocessor = COVID19Preprocessor(args.hub_path, args.output_path, args.demo)
    ...
    try:
        ...
        logger.error(f"Failed to run preprocessing: {e}", exc_info=True)
        raise
