...
class FlusightDataProcessor:
    def __init__(self, data: pd.DataFrame, locations_data: pd.DataFrame, target_data: pd.DataFrame):
        ...
        logger.info("Building individual FluSight JSON files...")
    ...
    def _build_ground_truth_key(self, df: pd.DataFrame) -> dict: # assumes time-series.csv
        """Build ground_truth key of an invidual JSON file"""
        ...
        filtered_target_data = self.target_data[
            ...
            (self.target_data['target'] == 'wk inc flu hosp')
        ].copy()
        ...
        filtered_target_data['target_end_date'] = pd.to_datetime(filtered_target_data['target_end_date'])

        # Select only most recently updated record for each week
        truth = filtered_target_data.sort_values('as_of').drop_duplicates(subset=['target_end_date'], keep='last')

        # Filter for the relevant flu season (can change)
        truth = truth[truth['target_end_date'] >= pd.Timestamp('2023-10-01')]

        # Sort before creating lists
        truth.sort_values('target_end_date', inplace=True)

        # Build and return final ground_truth dict
        ground_truth = {
            "dates": truth['target_end_date'].dt.strftime('%Y-%m-%d').tolist(),
            "wk inc flu hosp": truth['observation'].tolist(),
        }
        return ground_truth      


    def _build_forecasts_key(self, df: pd.DataFrame) -> dict:
        """Builds the forecasts key of an individual JSON file"""
        forecasts = {}
    ...
    def _build_metadata_file(self, all_models: list[str]) -> dict: 
        """Build single output metadata.json file (one per dataset output)"""
